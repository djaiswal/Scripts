...
## Loading radiation
shortR <- nc_open(paste("dswrf.",k,".nc",sep=""))## radiation will come form NLDAS

## Loading temperature
airT <- nc_open(paste("air.sfc.",k,".nc",sep=""))
## Loading relative humidity
rhum <- nc_open(paste("rhum.2m.",k,".nc",sep=""))
## Loading windspeed
winds <- nc_open(paste("uwnd.10m.",k,".nc",sep=""))
## Loading precip
precip <- nc_open(paste("apcp.",k,".nc",sep=""))
## vegetation
veg <- nc_open(paste("veg.",k,".nc",sep=""))
## Soil moisture
soilm <- nc_open(paste("soilm.",k,".nc",sep=""))
...
for(i in 1:dim(lat)[1]){
 ...
 filename=paste("/home/groups/ebimodeling/met/NARR_25_12_2016/ProcessedNARR/",k,formatC(i,width=3,flag=0),formatC(j,width=3,flag=0),".RData",sep="")
 soilfilename= paste("/home/groups/ebimodeling/met/NARR_25_12_2016/ProcessedNARR/soilm/",k,formatC(i,width=3,flag=0),formatC(j,width=3,flag=0),".RData",sep="")
